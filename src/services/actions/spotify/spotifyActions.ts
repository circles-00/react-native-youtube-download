...
import {
  spotify_categories_endpoint,
  spotify_tracks_for_playlist_endpoint
...
export const getSpotifyCategories:
  ...
  () => async (dispatch: Dispatch<ISetSpotifyCategoriesAction | any>) => {
    try {
      ...
      const { data } = await axios.get(spotify_categories_endpoint.path)
      dispatch(setSpotifyCategories(data))
...
export const setSpotifyCategories: ActionCreator<
  ISetSpotifyCategoriesAction
...
export const getSpotifyTracksForPlaylist:
  ...
  | ActionCreator<void> =
  (playlistId: string) =>
  async (dispatch: Dispatch<ISetSpotifyTracksForPlaylist | any>) => {
    try {
      dispatch(setSpinner(true))
      const res = await axios.get(spotify_tracks_for_playlist_endpoint.path, {
        params: { playlistId }
      })
