...
import { ISpotifyCatalogs } from './interfaces/definitions';

const { SPOTIFY_BASE_URL } = process.env;

export class CatalogsPackage implements ISpotifyCatalogs {
  handleError: (
    err: any,
  ...
  async getCategoryPlaylists(category: string, numOfTries = 3) {
    try {
      const result = await this.axiosInstance.get(
        `${SPOTIFY_BASE_URL}browse/categories/${category}/playlists`,
      );
      return result.data.playlists.items;
    } catch (err) {
      this.handleError(
        err,
        () => this.getCategoryPlaylists(category),
        numOfTries,
      );
    }
  }
